      ...
      "params": [
        {
          ...
          "enumOptions": [
            ...
            {
              "title": "Cabbage Sweety",
              "value": "Cabbage Sweety"
            },
            {
              "title": "Lindainlove",
              "value": "Lindainlove"
            ...
            },
            {
              ...
              "value": "Karesi ni Baretara Kaisan"
            },
            {
              "title": "k production film",
              "value": "k production film"
            },
            {
              "title": "798DS",
              "value": "798DS"
            ...
            {
              "title": "aiwanxiongxiong",
              "value": "aiwanxiongxiong"
            },
            {
              "title": "andmlove",
              "value": "andmlove"
            ...
            {
              "title": "Qiobnxingcaiii",
              "value": "Qiobnxingcaiii"
            },
            {
              "title": "SakuraCandy",
              "value": "SakuraCandy"
            ...
            {
              "title": "thelittlejuicer",
              "value": "thelittlejuicer"
            },
            {
              "title": "TLMS_SVJ",
              "value": "TLMS_SVJ"
...
function extractVideoInfo($, element, viewkey) {
    ...
    // 构建视频详情页URL
    var link = "https://cn.pornhub.com/view_video.php?viewkey=" + viewkey;
...
async function getSearchResults(params) {
    const searchQuery = String(params.keyword || params.search_query || '').trim();
    if (!searchQuery) return [];
    const page = Math.max(1, Number(params.page) || 1);  // 确保页码正确
    const sortBy = params.sort_by || "";
    const searchType = params.search_type || "no";  // 获取是否开启精准搜索的设置

    const formattedQuery = encodeURIComponent(searchQuery.toLowerCase()).replace(/%20/g, '+');
...
function parseSearchResults($, $list, searchQuery, formattedQuery, searchType) {
    ...
    $list.each((i, el) => {
        ...
        result.push({
            id: link || `https://cn.pornhub.com/view_video.php?viewkey=${vkey}`,
            type: "url",
            mediaType: "movie",
            title: title,
...
function extractRecommendedVideos(htmlContent, maxCount = 10) {
    ...
    $("li[data-video-vkey]").slice(0, maxCount).each(function (i, element) {
        ...
        const description = authorName ? `作者：${authorName}` : "";
        result.push({
            id: `https://cn.pornhub.com/view_video.php?viewkey=${vkey}`,
            type: "url",
            mediaType: "movie",
            title: title,
            coverUrl: coverUrl,
...
async function loadDetail(link) {
    try {
        ...
        const mainCover = $root("meta[property='og:image']").attr("content") || "";
        const mainDescription = $root("meta[property='og:description']").attr("content") || "";
        ...
        const result = {
            ...
            coverUrl: mainCover || undefined,
            posterPath: mainCover || undefined,
            backdropPath: mainCover || undefined,
            playerType: "system",
            customHeaders: {
            ...
            title: $root("meta[property='og:title']").attr("content") || "视频播放",
            duration: 0,
            formats: m3u8Data.formats,
            description: mainDescription || (mainAuthor ? `作者：${mainAuthor}` : ""),
