...
async function search(params = {}) {
  const keywordText = String(params.keyword || "").trim();
  ...
  return await loadPage({ ...params, from: page > 1 ? page : undefined, url });
}
  ...
  for (const sectionElement of sectionElements) {
    ...
    if (itemElements && itemElements.length > 0) {
      for (const itemElement of itemElements) {
        ...
        if (url && /^https?:\/\/(?:www\.)?jable\.tv\//i.test(url)) {
...
async function loadDetail(link) {
  ...
  const hlsMatch = response.data.match(/var\s+hlsUrl\s*=\s*["']([^"']+)["']/i);
  const hlsUrl =
    (hlsMatch && hlsMatch[1]) ||
    $("video source[src*='.m3u8'], source[src*='.m3u8']").first().attr("src") ||
    "";
  if (!hlsUrl) {
  ...
  const item = {
    ...
    durationText: videoDuration || undefined,
    link: link,
    playerType: "system",
    customHeaders: {
      "Referer": link,
  ...
  const items = sections.flatMap((section) => section.childItems);
  if (items.length > 0) {
    item.relatedItems = items.slice(0, 20);
  }
